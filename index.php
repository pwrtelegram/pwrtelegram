...
if(strtolower($_SERVER['REQUEST_URI']) == "/getFile" && $_REQUEST['file_id'] != "") {
	...
	if($response["ok"] == false && preg_match("/\[Error\]: Bad Request: file is too big\[size:/", $response["description"])) {
		$databasestmt = $pdo->prepare("SELECT * FROM dl WHERE file_id=? DESC LIMIT 1;");
		$database = $databasestmt->execute(array($_REQUEST['file_id']));
		if(checkurl("https://storage.n2ogram.daniil.it/" . hash('sha256', $token) . $database["file_path"])) {
			$newresponse = $database;
			$newresponse["ok"] = true;
		} else {
