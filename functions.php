...
function upload($file, $name = '', $type = '', $forcename = false, $oldparams = [])
{
    ...
    if (file_exists($file)) {
        ...
        if (preg_match('|^http(s)?://'.$pwrtelegram_storage_domain.'/|', $file)) {
            ...
            $select_stmt->execute([preg_replace('|^http(s)?://'.$pwrtelegram_storage_domain.'/|', '', $file), $me]);
    ...
    if ($file_id == '') {
        ...
        if (false && $size < 50000000) {
            ...
            curl_setopt($ch, CURLOPT_POSTFIELDS, ['chat_id' => $botusername, $type => new \CURLFile($path)]);
            $result = json_decode(curl_exec($ch), true);
            curl_close($ch);
            if ($result['ok'] == true) {
